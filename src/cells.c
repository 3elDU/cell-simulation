...
void CELLS_UpdateCell(struct CELLS_State* state, struct CELLS_Cell cell) {
	...
	if (cell.alive) {
		...
		if (cell.direction == LEFT) facingX--;
		else if (cell.direction == RIGHT) facingX++;
		else if (cell.direction == UP) facingY--;
		else if (cell.direction == DOWN) facingY++;

		if (facingX == -1) facingX = state->width-1;
		else if (facingX == state->width) facingX = 0;

		if (facingY == -1) facingY = state->height-1;
		else if (facingY == state->height) facingY = 0;
		...
		switch(cell.genome[cell.currentInstruction].command) {
			...
			case TURN_LEFT:
				if (cell.direction-1 == -1) cell.direction = DOWN;
				else cell.direction--;
			...
			case TURN_RIGHT:
				if (cell.direction+1 == 4) cell.direction = LEFT;
				else cell.direction++;
			...
			case MAKE_CHILD:
				...
				if (cell.energy < REPRODUCTION_REQUIRED_ENERGY) break;
				...
				if (!CELLS_GetCell(state, facingX, facingY)->alive) {
					...
					for (int i = 0; i < GENOME_LENGTH; i++) {
						...
						if (CELLS_Random(1, 100) <= MUTATION_PERCENT) {
							...
							if (colorToChange == 0) child.r += CELLS_Random(-16, 16);
							if (colorToChange == 1) child.g += CELLS_Random(-16, 16);
							if (colorToChange == 2) child.b += CELLS_Random(-16, 16);
						}
						else child.genome[i] = cell.genome[i];
					}
...
struct CELLS_State* CELLS_Init(const unsigned width, const unsigned height) {
	...
	if (state->cells == NULL) goto failed;

	state->updateOrder = calloc(width * height, sizeof(unsigned));
	if (state->updateOrder == NULL) goto failed;
...
struct CELLS_Cell* CELLS_GetCell(const struct CELLS_State* state, const unsigned x, const unsigned y) {
	if (x >= state->width || y >= state->height) return NULL;
...
unsigned CELLS_CountAliveCells(const struct CELLS_State* state) {
	...
	for (int i = 0; i < state->width; i++) {
		for (int j = 0; j < state->height; j++) {
			if (CELLS_GetCell(state, i, j)->alive) count++;
		}
	}
